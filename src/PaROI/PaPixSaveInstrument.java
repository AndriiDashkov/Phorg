...
public class  PaPixSaveInstrument extends PaZoomRectInstrument {
	 ...
	 */
	public  PaPixSaveInstrument(PaInstrumentsRoiPanel parent, PaPixSaveButton but) {
		
		super(parent, but);
		
		m_keepActivated = but.isKeepInstrumentActivated();
	...
	public Image getResultView(MouseEvent e,float xScale, float yScale, BufferedImage sourceImage) {
		...
		if(m_startPoint != null) {
			...
			try {
				...
				PaAlgoConvert.savePixelsToFile(im,  rec, ((PaPixSaveButton) m_button).isHSIsaveSelected(), 
						((PaPixSaveButton) m_button).isRGBsaveSelected(), ((PaPixSaveButton) m_button).getPrintWriter());
				
				
				return null;//we don't change the image by this instrument, so in this case the null must be returned
			}
			finally {
	...
	public void clearOperation() {
		...
		((PaPixSaveButton) m_button).closeWriter();
	...
	private void createCustomCursor() {
		...
		Image image = toolkit.getImage(PaUtils.get().getIconsPath() + "pahandcursor.png");
		
		//image paredeyecursor.png size 32x32 - hot point is 15,15
	...
	public boolean filterMouseEvent(MouseEvent e) {
		...
		if (e.getID() ==  MouseEvent.MOUSE_DRAGGED  ) {
			...
			bottomRightPoint.x = m_startPoint.x + m_currentSize.width/2;
			
			bottomRightPoint.y = m_startPoint.y - m_currentSize.height/2;
