 ...
 * @author avd
 * Ѕазовое окно m_baseWindow это отражение m_viewImage на реальное изображение; m_viewImage может быть больше чем
 * viewport; размер m_baseWindow все врем€ синхронизирован с размером m_viewImage(но не одинаков)
 */
public class PaScrollView extends JScrollPane {
	...
	/**
	 * ƒает новый размер дл€ base Window дл€ нового вход€щего rec - понимаетс€ как новый размер
	 * дл€ view Image; пересчет ведетс€ через старый размер viewImage и baseWindow
